...
- 활성도: 두 축의 max — 어느 한 축이라도 '아직 활발'이면 활발로 본다.
  ...
  잊힌 것으로 잘못 잡힌다 — 잊힘은 '정점이 오래됨 AND 현재 주목이 정점 대비 미미함'
  둘 다 필요하므로 활성은 그 부정인 max로 결합한다. (sies.rank의 min과 방향이 반대인
  이유: 저쪽은 '잊힘 축들의 OR', 여기는 '활성 축들의 OR'.)
...
BAND_K = 50.0
RECENT_WINDOW_MONTHS = 12  # '지금도 보이는가'(A_now)의 관측 창.
...
class PersonScore:
    ...
    activity_now: float = float("nan")    # 현재 주목 축(최근 평균/정점)
...
def score_candidates(
    ...
    survivors: list[tuple[Candidate, str | None, int, float, float, float, float]] = []
    for c in candidates:
        ...
        if max_activity is not None and a >= max_activity:
            continue
        survivors.append((c, peak_month, pv, a_time, a_now, a, rmean))

    if not survivors:
        return []

    peak_views = np.array([pv for _, _, pv, _, _, _, _ in survivors], dtype=float)
    ...
    for (c, peak_month, pv, a_time, a_now, a, rmean), w in zip(survivors, weights):
